...
export default function DashboardPage() {
  ...
  async function redeem() {
    ...
    try {
      const response = await fetch("/api/redeem", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ code: value }) });
      const payload = await response.json();
      if (!response.ok || !payload.data?.accessPath) throw new Error(payload.message || "兑换失败");
      const accessPath = payload.data.accessPath.replace(/^\/+/, "");
      const basePath = import.meta.env.BASE_URL.replace(/\/?$/, "/");
      window.location.href = `${basePath}${accessPath}`;
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "兑换失败，请检查卡密");
    }
  }
