...
- `dashboard/`：React + Vite 用户端，构建后部署到 `/dashboard`
- `admin/`：管理端前端（React + Vite + Ant Design Pro Components），构建产物由 `server/` 打进 Go 二进制，部署到 `/admin`
- `server/`：Go 后端（标准库 + SQLite），提供 `/api`、`/sub` 接口并托管全部静态资源
- `CNAME`：当前域名记录 `netx.beisi.tech`

三个前端分别独立构建，产物统一打包进 Go 服务：根路径提供 `landing/`，`/dashboard` 提供 React 用户端，`/admin` 提供 Vue 管理端。浏览器通过 `/api` 调用 Go 后端，不需要 Next.js、Node 运行时或 Nginx 网关。
...
开发服务启动后访问 `http://localhost:3000/dashboard`，接口请求会代理到 `http://localhost:8000`。
...
脚本会构建一个包含落地页、React 用户端、Vue 管理端和 Go 后端的镜像，并启动单一服务。入口地址为 `http://localhost:8000`；脚本优先读取 `deploy/.env`，该文件不存在时使用本地默认管理员账号 `admin` 和密码 `admin123456`。
...
服务器需安装 Docker Engine 和 Compose 插件，部署用户需有权限直接执行 `docker`。SQLite 数据保存在 Docker named volume `netx_server-data` 中，更新容器不会删除数据。

## 管理端与后端

管理端和用户端都是 Go 后端的一部分：`admin/` 的构建产物输出到 `server/web/dist/admin`，`dashboard/` 的构建产物输出到 `server/web/dist/dashboard`，由 Go 通过 `go:embed` 打进二进制，由同一个进程提供 `/`、`/dashboard`、`/admin` 静态页与 `/api` 接口。
...
make admin-install        # 首次安装管理端依赖（使用 npm）
make server-dev           # 终端一：Go 后端，监听 8000
make admin-dev            # 终端二：管理端开发服务
```

`server/.env` 已被 Git 忽略，由 `make server-dev` 加载；直接在 `server/` 运行 `go run .` 时仍需自行设置环境变量。生产运行只读取进程环境变量。

浏览器访问 `http://localhost:5173/admin/`，接口请求由 Vite 代理到 8000，管理端 API 路径统一为 `/api/admin`。
